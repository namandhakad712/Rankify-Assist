"""
Browser Automation - Combined Tuya MCP Bridge
All-in-one Hugging Face Space deployment

Runs BOTH:
1. Tuya Client - Connects to Tuya Platform
2. MCP Server - Handles browser automation tools
...
async def execute_browser_command(
    command: Annotated[str, Field(description="Natural language command to execute in browser")]
) -> str:
    """Execute a browser command via Rankify extension"""
    log_message(f"Tool called: execute_browser_command('{command}')", "MCP-SERVER")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                return f"✅ Command '{command}' sent to browser! (ID: {command_id})"
            else:
                log_message(f"❌ Failed: {response.status_code}", "MCP-SERVER")
                return f"❌ Failed to send command: {response.text}"
...
async def health_check() -> str:
    ...
    return "✅ Browser Automation MCP Server is online and healthy!"
...
async def start_tuya_client():
    """Start Tuya MCP client and connect to Tuya Platform"""
    ...
    try:
        ...
        MCP_SERVER_URL = "http://localhost:7860/mcp"  # Our own MCP server!
        ...
        if not all([TUYA_ENDPOINT, TUYA_ACCESS_ID_SDK, TUYA_ACCESS_SECRET]):
            ...
            log_message("Set: MCP_ENDPOINT, MCP_ACCESS_ID, MCP_ACCESS_SECRET", "TUYA-CLIENT")
            return
        ...
        log_message("✅ Successfully connected to Tuya Platform!", "TUYA-CLIENT")
        log_message("✅ Forwarding requests to local MCP server", "TUYA-CLIENT")
        log_message("🎧 Listening for AI Workflow requests...", "TUYA-CLIENT")
        
        # Keep listening
        await tuya_client.start_listening()
        
    except Exception as e:
        log_message(f"❌ Connection error: {e}", "TUYA-CLIENT")
        tuya_client_connected = False
        import traceback
        log_message(f"Traceback: {traceback.format_exc()}", "TUYA-CLIENT")

def start_tuya_client_background():
    """Start Tuya client in background thread"""
...
log_message("🎬 Browser Automation MCP Bridge Starting...", "SYSTEM")
log_message("=" * 60, "SYSTEM")
...
# Create UI
with gr.Blocks(title="Browser Automation MCP Bridge", theme=gr.themes.Soft(primary_hue="blue")) as demo:
    
    gr.Markdown("# 🌐 Browser Automation - Tuya MCP Bridge")
    gr.Markdown("**All-in-One Deployment:** Tuya Client + Browser Automation MCP Server")
    ...
    gr.Markdown("""
    ### 🎯 This Space Does:
    - ✅ Connects to Tuya Platform (persistent connection)
    - ✅ Handles browser automation tools
    - ✅ Forwards commands to Cloud Bridge
    - ✅ Monitors everything in real-time
    
    ### 📝 Required Environment Variables:
    Set in **Settings → Variables and secrets**:
    - `CLOUD_BRIDGE_URL` - Cloud Bridge URL  
    - `MCP_API_KEY` - Cloud Bridge API key
    - `MCP_ENDPOINT` - Tuya Platform endpoint
    - `MCP_ACCESS_ID` - Tuya Access ID
    - `MCP_ACCESS_SECRET` - Tuya Access Secret
    """)
