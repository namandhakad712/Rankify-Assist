...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center">
          <label
            ...
            className="w-24 text-sm font-medium text-primary">
          ...
          <div className="flex items-center">
            ...
            <div className="flex flex-1 items-center space-x-2">
              <input
                ...
                className="flex-1 accent-current text-primary h-1 appearance-none rounded-full bg-[var(--input-bg)]"
              />
  ...
  const handleFetchModels = async (providerId: string) => {
    const config = providers[providerId];
    if (!config || (!config.apiKey && config.type !== ProviderTypeEnum.Ollama)) {
      alert('Please enter an API Key first (unless using Ollama).');
      return;
    }
    ...
    if (config.type === ProviderTypeEnum.Ollama) {
      ...
      fetchUrl = `${base}/api/tags`;
    } else {
      // Standard OpenAI compatible
      let base = config.baseUrl?.replace(/\/$/, '');

      // Defaults if missing (though for custom it's required)
      if (!base) {
        if (config.type === ProviderTypeEnum.Groq) base = 'https://api.groq.com/openai/v1';
        else if (config.type === ProviderTypeEnum.Cerebras) base = 'https://api.cerebras.ai/v1';
        else if (config.type === ProviderTypeEnum.OpenRouter) base = 'https://openrouter.ai/api/v1';
        else if (config.type === ProviderTypeEnum.Gemini) base = 'https://generativelanguage.googleapis.com/v1beta/openai';
      }
    ...
    if (!fetchUrl) {
      alert('Could not determine API endpoint. Please check Base URL.');
      return;
    }
    ...
    try {
      ...
      setProviders(prev => {
        ...
        const uniqueModels = [...new Set([...currentModels, ...fetchedModels])];
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <div className="flex justify-end mb-1">
                            <button
                              ...
                              className={`flex items-center text-xs ${isDarkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-600 hover:text-blue-800'}`}
                            >
                              <FiDownload className="mr-1" /> Auto-Fetch Models
                            </button>
                          </div>
